...
with check (
  ...
  and (
    note_comments.parent_comment_id is null
    or exists (
      select 1
      from public.note_comments parent
      where parent.id = note_comments.parent_comment_id
        and parent.note_id = note_comments.note_id
    )
  )
);
...
with check (
  ...
  and (
    question_answers.parent_answer_id is null
    or exists (
      select 1
      from public.question_answers parent
      where parent.id = question_answers.parent_answer_id
        and parent.question_id = question_answers.question_id
    )
  )
);
